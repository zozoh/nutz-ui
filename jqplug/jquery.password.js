...
(function($) {
    ...
    function _do_check(selection, opt) {
        ...
        // 先检测密码强度
        var m = !pwd || pwd.length < 6 ? 0 : /[0-9]/.test(pwd) * 1 + /[a-z]/.test(pwd) * 1 + /[A-Z]/.test(pwd) * 1 + /[^0-9a-zA-Z]/.test(pwd) * 1;
        $(selection.find(".pwd-tester .pwd-test-cell").removeClass("pwd-test-on")[m]).prevAll(".pwd-test-cell").andSelf().addClass("pwd-test-on");

        // 如果两者一致 ...
        if (pwdRe && pwd && pwdRe == pwd) {
            jPwdRe.removeClass("pwd-invalid");
            selection.find(".pwd-verifier").empty();
            if (typeof opt.on_ready == "function") opt.on_ready.call(jPwdNew, pwd);
        }
        // 否则 ...
        else if (pwdRe) {
            jPwdRe.addClass("pwd-invalid");
            selection.find(".pwd-verifier").text(opt.msg("pwd.unmatch"));
            if (typeof opt.on_unmatch == "function") opt.on_unmatch.call(selection, jPwdNew, jPwdRe);
        }
    }
    // 扩展插件
    $.fn.extend({
